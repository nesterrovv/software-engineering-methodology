...
const RegulatoryReportsPage = () => {
    ...
    const handleGenerate = async (event: FormEvent) => {
        ...
        setError("");
        try {
            const data = await apiRequest<ReportResponse>(baseUrl, token, "/api/incident/reports/regulatory", {
                method: "POST",
                body: JSON.stringify({
            ...
            setError("Не удалось создать отчёт для регуляторов.");
        }
    };

    return (
        <PageShell
            title="Генерация отчётов для регуляторов"
            subtitle="Создание и экспорт отчётов, требуемых для отчётности перед регуляторами."
        ...
        >
            ...
            <section>
                ...
                <div className="card-grid">
                    ...
                    {reports.length === 0 ? (
                        <div className="card">
                            <h3>Отчёт по нарушениям и инцидентам</h3>
                            <p className="data">Сформируйте отчёт для выгрузки.</p>
